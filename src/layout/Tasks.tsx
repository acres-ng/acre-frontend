...
<div className="px-4">
      {taskData.map((task, index) => (
        <div key={index} className="bg-white border rounded-xl shadow-md p-4 mb-4">
          <div className="flex justify-between mb-2">
            <div>
              <span className="font-bold text-lg mb-2">{task.title}</span>
              <span className={`rounded-full ${getStatusColor(task.status)} py-1 px-2 ml-16`}>
                {task.status}
              </span>
              <span className="text-gray-500 block">{task.description}</span>
            </div>
           
          </div>
          <span className="text-gray-500">Due {task.due_date || "N/A"}</span>
          <div className="flex items-center">
            <span>Assignee to</span>
            {/* Placeholder for an avatar */}
            <img
              className="w-8 h-8 rounded-full border-2 border-white ml-2"
              src={task.assignee ? `url/to/assignee/avatar/${task.assignee}` : "https://via.placeholder.com/150"}
              alt="Avatar"
            />
          </div>
        </div>
